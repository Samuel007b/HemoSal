...
function Viagens() {
  ...
  useEffect(() => {
    ...
    setSubtitulo(`${total} ${total === 1 ? 'viagem' : 'viagens'}`)
  }, [viagens.length, setSubtitulo])
  ...
  return (
    <div className="viagens-conteudo">
      <div className="viagens-topo">
          ...
          <table className="viagens-tabela">
            ...
            <tbody>
              {viagensPagina.map((viagem) => {
                const vagasLivres = viagem.limiteVagas - viagem.doadores.length
                return (
                  <tr key={viagem.id}>
                    <td>{formatDateTime(viagem.dataSaida)}</td>
                    <td>{viagem.doadores.length}</td>
                    <td>{viagem.limiteVagas}</td>
                    <td><span className="badge-vagas-livres">{vagasLivres} vaga(s) livre(s)</span></td>
                    <td>
      ...
      {detalheId && (
        <ViagemDetalhesModal
          ...
          onEditar={(viagem) => { setDetalheId(null); setFormModal({ modo: 'editar', viagem }) }}
          onExcluido={() => { setDetalheId(null); carregarViagens() }}
        />
      )}
